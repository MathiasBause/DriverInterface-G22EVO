...
volatile byte buttonsData = 0b00000000; //Creamos un byte que contendrá la información de los botones a transmitir por CAN.
...
void start() {
  ...
  //CAN.setMode(MCP_NORMAL); //POSIBLE ERROR
...
void writeCanBus(int pos){
  // send data:  ID = 0x02, Standard CAN Frame, Data length = 1 byte, 'data' = data byte to send
  buttonsData = buttonsData || (0x01 << (8-pos));  
  ...
  buttonsData = 0b00000000;
  delay(500);
  sndStat = CAN.sendMsgBuf(0x02, 0, 1, buttonsData); //Revisar ID
  if(sndStat == CAN_OK){
    Serial.println("Message Sent Successfully!");
  } else {
    Serial.println("Error Sending Message...");
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(UPSHIFT), upshift, HIGH);
  attachInterrupt(digitalPinToInterrupt(DOWNSHIFT), downshift, HIGH);
  Serial.begin(115200);
  start();
}
